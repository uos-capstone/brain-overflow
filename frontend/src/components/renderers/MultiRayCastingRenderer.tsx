...
fn fsMain(in: VertexOut) -> @location(0) vec4<f32> {
  ...
  let iso1 = tf.dMin1;       // iso-surface 값
  let width1 = tf.alpha1;    // 허용 오차 범위 (슬라이스 두께)
  let color1 = vec3f(1.0, 0.3, 0.2); // isosurface 색

  let iso2 = tf.dMin2;       // iso-surface 값
  let width2 = tf.alpha2;    // 허용 오차 범위 (슬라이스 두께)
  let color2 = vec3f(0.2, 0.8, 1.0); // isosurface 색
  ...
  loop {
    ...
    if (all(voxelPos >= vec3f(0.01)) && all(voxelPos <= vec3f(0.99))) {
      ...
      if (abs(d - iso1) < width1) {
        ...
        let shadedColor = vec3f(color1 * lighting);
        return vec4f(shadedColor, 1.0); // 바로 종료
      }
      else if (abs(d - iso2) < width2) {
        // gradient = normal
        let eps = vec3f(1.0) / uniforms.volumeDims;
        let dx = textureSampleLevel(volume, volumeSampler, texCoord + vec3f(eps.x, 0.0, 0.0), 0.0).r -
                 textureSampleLevel(volume, volumeSampler, texCoord - vec3f(eps.x, 0.0, 0.0), 0.0).r;
        ...
        let shadedColor = vec3f(color2 * lighting);
        return vec4f(shadedColor, 1.0); // 바로 종료
      }
    }
  ...
  return vec4f(0.0); // 아무것도 안 만났을 경우
}

`;

export class MultiRayCastingRenderer {
