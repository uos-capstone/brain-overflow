...
declare global {
    interface Window { SockJS: any; } // SockJS 생성자 타입 (더 정확한 타입 지정 가능)
}
...
export function connectStomp(
    ...
    const socket = new window.SockJS(
        `https://api-brain-overflow.unknownpgr.com/ws?token=${encodeURIComponent(token)}`
    );

    stompClient = Stomp.over(socket);
    stompClient.debug = (str: string) => { console.log(`[STOMP_DEBUG] ${str}`); };
    ...
    stompClient?.subscribe(`/user/queue/chat`, (msg: StompMessage) => {
        try {
            const messagePayload: ServerChatMessage = JSON.parse(msg.body);
                console.log(`[/user/queue/chat] 메시지 수신:`, messagePayload);
...
export function subscribeToRoomMessages(roomId: string, callback: (message: ServerChatMessage) => void): () => void {
...
export function sendChatMessage(roomId: string, content: string): void {
    if (stompClient && stompClient.connected) {
        ...
        console.log(`[STOMP] 메시지 전송 to ${chatDestination} (roomId: ${roomId}):`, content);
    } else {
